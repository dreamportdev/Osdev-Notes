...
There are also more complex types of locks like semaphores and mutexes. A semaphore is a more complex kind of lock, as it's usually provided to user programs by the kernel. This makes the semaphore quite expensive to use, but it allows the kernel to make decisions when a program wants to acquire/release the semaphore. For example if a thread tries to acquire a semaphore that is taken, the kernel might put the current thread to sleep and reschedule. When another thread releases the semaphore, the kernel will wake up the sleeping thread and have it scheduled next. Semaphores also usually allow for more than a single thread to hold the lock at a time.
