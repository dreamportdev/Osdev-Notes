...
The first step needed to configure the LAPIC is getting access to it. The apic registers are memory mapped registers, to get their location we need to read the MSR register that contains its base address, using the __rdmsr__ assembly instruction. This instruction read the content of the MSR register specified in `ecx`, the result is placed in `eax`.

In our case the register that we need to read is IA32_APIC_BASE and its value is 1Bh.
...
When the system boots up, the base address of the local APIC should be `0xfee0000`, and this should be the value tha we should read from from `rdmsr`. Now the APIC registers are accessed using an index that is an Offset from the base address. 

In the following paragraphs, when we talk about offset to access a register, we refer to the offset from the APIC base address.

Although there are many APIC registers we need to worry only about few of them: 

* The Spurious Interrupt Vector Register
* The EOI Register
* The LVT Timer register
* The local apic id and version registers

### Enabling the APIC And The Spurious Vector

To enable the APIC we need to use the enable/disable flag on the spurious interrupt vector register, this register is accessed at offset F0h:
...
* Bits 0-7: They determine the vector number (IDT entry) for the spurious vector interrupt generated by the APIC.
* Bit 8: To control if the local apic is enabled or disabled, if is 1 it means is enabled.
* Bit 9: It determine if Focus Processor Checking is enabled (1) or disabled (0). 

The Spurious Vector register is Writable only in the first 8 bits, the rest is read only. In order to enable the lapic we need to set the bit 8 to 1, and set-up a spurious vector entry for the idt. That can be any entry available in the table, and we need of course to set-up the corresponding idt entry with a function to handle it, but in our case we printing an error message is enough.

### Reading APIC id and version

LAPIC Id and version registers are located at offsets: 20h and 30h, the first one is r/w while the second one is read only. 

We need the id later when we are going to configure the IRQs with the io apic. Since there can be multiple local APIC on a system (we have one for every cpu/core), this number is used to uniquely identify them. The value after a system reset is 0. 

The local APIC version, contains few useful information, not really needed but it tell us hw many LVT entries (specified at bits 16:23)  it has (minus 1), this number change depending on the processor family (Xeon Processors have 6 LVT Entries, P6 Family 5 entries and the pentium processors have 4 entries.

### Local vector table

The local vector table allows the software to specify ho the local interrupts are delivered. 
There are 6 items in the Local Vector Table (LVT) starting from offset 320h to 370h:
