...
void create_endpoint(const char* name) {
    ...
    //add the endpoint to the end of the list
...
Now our endpoint has been added to the list! As always we omitted checking for errors, and we didn't check if an endpoint with this name already exists. In the real world, you'll want to handle these things.
...
The `ep_name` argument is the name of the endpoint we want to send to in this case. This leads nicely into the first thing we'll need to do: find an endpoint in the list with the matching name. This is a pretty standard algorithm, we just loop over each element comparing the endpoint's name with the one we're looking for.
...
You may want to return an error here if the endpoint can't be found, however, in our case we're simply discarding the message.
