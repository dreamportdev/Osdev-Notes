...
- How do you treat unused registers in a system call?
- What happens when a system call isn't found? Or not available?
- How do you pass arguments that doesn't fit in the 4 registers?
- How do you return data that doesn't fit in the 4 registers?
- If you support asynchronous operations, how do callback functions work?
...
The above function takes advantage of the fact the system V calling convention is the one used by GCC/Clang. If you're using a different compiler/callin convention, you will need to move the arguments into the registers yourself. This is as straightfoward as it sounds, but is left as an exercise for the reader.

This function also uses the `naked` attribute. If you're unfamiliar with attributes, they are discussed in the C language chapter. This particular attribute tells the compiler not to generate the entry and exit sequences for this function. These are normally very useful, but in our case are unnecessary.
