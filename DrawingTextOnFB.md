...
As already said the best place to look for a font if you are running on linux is to look into the folder `/usr/share/consolefonts`, to know the psf version i  suggest to use the tool *gbdfed*, import the font with it (use the File->Import->console font menu), and then go to *View->Messages*, the should be a message similar to the following: 
